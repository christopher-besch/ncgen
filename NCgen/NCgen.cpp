...
int main(int argc, char* argv[])
{
	...
	while (std::getline(file, filestr)) {
		...
		case strint("int"):
			append_content(parameter.substr(0, parameter.find(" ") + 3) + "int(" + parameter.substr(parameter.find(" ") + 3) + ")\n");
			break;

		case strint("str"):
			append_content(parameter.substr(0, parameter.find(" ") + 3) + "str('" + parameter.substr(parameter.find(" ") + 3) + "')\n");
			break;

		case strint("float"):
			append_content(parameter.substr(0, parameter.find(" ") + 3) + "float(" + parameter.substr(parameter.find(" ") + 3) + ")\n");
			break;
		...
		default:
			if (filestr.substr(0, filestr.find(" ")).find("js") != std::string::npos) {
				console_log("Ignoring JavaScript specific function: ");
				console_log("> " + filestr + "\n");
				break;
			}
			else {
				if (filestr.substr(0, filestr.find(" ")).find("html") != std::string::npos) {
